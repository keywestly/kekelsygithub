...
async function fa() {
  // Parallelize all API calls ? 4 batches
  const [ix, sc, sk, mc, ft, news] = await Promise.all([
    ...
    (async () => {
      const r = {};
      for (const [s,n] of [["XLK","??"],["SMH","???"],["XLF","??"],["XLV","??"],["XLE","??"],["XLP","?????"],["XLY","?????"]]) {
        const q = await fq(s); if (q) r[s] = { name: n, last: q.last, change: q.change, pct: q.pct };
      }
      return r;
    })(),
    ...
    (async () => {
      ...
      const [cl, gc, dx] = await Promise.all([sf("@CL.1","WTI??"), sf("@GC.1","??"), sf("@DX.1","????DXY")]);
